      ...
      return (
        <div className="space-y-3">
          {Object.entries(value).map(([key, dateObj], index) => {
            const startDate = dateObj.startDate || dateObj.start || dateObj[0] || "N/A";
            const endDate = dateObj.endDate || dateObj.end || dateObj[1] || "N/A";
            ...
            return (
              <div 
                key={key} 
                className="group relative bg-gradient-to-br from-white to-indigo-50/30 rounded-xl border border-indigo-100 p-4 hover:border-indigo-300 hover:shadow-md transition-all duration-200"
              >
                {/* Entry badge */}
                <div className="absolute top-3 right-3">
                  <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider bg-indigo-50 px-2 py-0.5 rounded-full">
                    Entry {index + 1}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-bold text-gray-900 truncate">{formattedStart}</p>
                    {startDate !== formattedStart && (
                      <p className="text-[10px] text-gray-400 mt-0.5 truncate">{startDate}</p>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-bold text-gray-900 truncate">{formattedEnd}</p>
                    {endDate !== formattedEnd && (
                      <p className="text-[10px] text-gray-400 mt-0.5 truncate">{endDate}</p>
                    )}
                  </div>
                </div>
                
                {/* Duration badge */}
                {startDate !== "N/A" && endDate !== "N/A" && startDate !== endDate && (
