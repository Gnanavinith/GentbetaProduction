...
export default function ActiveFormsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [formsRes, templatesRes] = await Promise.all([
        formApi.getForms({ limit: 1000, page: 1 }),
  ...
  const onConfirmApproval = async (approverId) => {
    ...
    try {
      ...
      if (response.success) {
        toast.success(response.message || `Form(s) assigned successfully to the employee!`);
        setSelectedForms([]);
        setIsApproverModalOpen(false);
      } else {
        const errorMessage = response.message || "Failed to assign forms";
        toast.error(errorMessage.includes("allshow") ? "Failed to assign forms. Please check your selection and try again." : errorMessage);
      }
    } catch (err) {
  ...
  const handleClearSelection = () => setSelectedForms([]);
  ...
  const handleDeleteTemplate = async (itemId) => {
    ...
    try {
      if (isForm) await formApi.deleteForm(itemId);
      else await templateApi.deleteTemplate(itemId);
      fetchData();
    } catch (err) {
      logError(`Delete ${isForm ? 'form' : 'template'}`, err);
      toast.error(err.response?.data?.message || `Failed to delete ${isForm ? 'form' : 'template'}`);
    }
  };
  ...
  const handleArchiveTemplate = async (itemId) => {
    ...
    try {
      if (isForm) await formApi.archiveForm(itemId);
      else await templateApi.archiveTemplate(itemId);
      fetchData();
    } catch (err) {
  ...
  const handleRestoreTemplate = async (itemId) => {
    ...
    try {
      if (isForm) await formApi.restoreForm(itemId);
      else await templateApi.restoreTemplate(itemId);
      fetchData();
    } catch (err) {
  ...
  const filteredItems = [...regularForms, ...templateForms]
  ...
  const getStatusBadge = (status) => {
    ...
    switch (s) {
      case "DRAFT": return <span className="px-2 py-0.5 bg-gray-100 text-gray-600 text-[10px] font-bold rounded-full uppercase">Draft</span>;
      case "IN_APPROVAL": return <span className="px-2 py-0.5 bg-amber-100 text-amber-700 text-[10px] font-bold rounded-full uppercase">In Approval</span>;
      ...
      case "ARCHIVED": return <span className="px-2 py-0.5 bg-gray-200 text-gray-700 text-[10px] font-bold rounded-full uppercase">Archived</span>;
      default: return <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-[10px] font-bold rounded-full uppercase">{s}</span>;
    }
  };
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100">
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  {/* ── Row number column ── */}
                  <th className="px-3 py-2.5 text-xs font-bold text-gray-500 uppercase tracking-wider w-10 text-center">
                    #
                  </th>
          ...
          {filteredItems.length === 0 && (
            <div className="text-center py-12 bg-gray-50/30">
              ...
              <h3 className="text-gray-900 font-medium text-sm">No active forms found</h3>
              <p className="text-gray-500 text-xs">Create a new form to get started.</p>
            </div>
          )}
        </div>
      )}
